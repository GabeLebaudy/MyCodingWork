...
def getStockReqInfo(stock_abbr):
    ...
    search_bar_element.send_keys(stock_abbr)
    time.sleep(1)
    ...
    while not found:
        ...
        try:
            ...
            if stock_abbr == list_option_element.text:
                found = True
        except:
            break
...
def main():
    abbr = "SLV"
    info = getStockReqInfo(abbr)
    if not info:
        return False
    
    response = getJsonData(info)
    if not response:
        return False
    ...
    output_path = os.path.join(os.path.dirname(__file__), "{}_main_data.csv".format(abbr))
    ...
    if has_dividends:    
        dividends_output = os.path.join(os.path.dirname(__file__), "{}_dividends.csv".format(abbr))
